...
A transfer hook runs on every transfer of its mint. A buggy or malicious hook can block transfers, grief integrators, or freeze a token. This file is the checklist for writing and for auditing a hook. The example program in `examples/transfer-hook-allowlist` implements every check below.
...
5. Validate account linkage. Check that each token account's `mint` field matches the expected mint. Do not trust account order alone.
