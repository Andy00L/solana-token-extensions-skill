...
1. If the `examples/mint-inspector` tool is present in the repo, run it and use its output:
   ...
   It decodes the extension set and prints findings, conflicts, a CEX, DEX, and wallet posture, and a renounce-to-remediate path (what the verdict becomes once each live authority is renounced). Add `--json` for machine output. Pass several addresses to triage a set at once (a per-mint verdict plus an aggregate).
